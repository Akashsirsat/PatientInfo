...
class Database:
    ...
    def Search(self, id):
        self.row_count=self.dbCursor.execute("SELECT * FROM patient_table WHERE id = ?", (id,))
        searchResults = self.dbCursor.fetchall()
        return searchResults

    def Delete(self, id):
        self.id=id
        self.valueErrorMessage="No such Patient"
        obj=Database()
        self.data=obj.Search(id)

        self.li = list()
        for self.var in self.data:
            self.li = self.var
        # print(len(self.li))
        if (len(self.li) == 0):
...
class UpdateWindow:
    def __init__(self, id):
        ...
        self.li = list()
        for self.var in self.searchResults:
            self.li = self.var
        # print(len(self.li))
        #self.up = UpdateWindow()
        if (len(self.li) == 0):
            tkinter.messagebox.showerror("Value Error", self.valueErrorMessage)
        else:
